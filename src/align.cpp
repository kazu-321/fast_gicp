...
public:
  AlignNode() : Node("align_node") {
    source_sub_ =
      this->create_subscription<sensor_msgs::msg::PointCloud2>(TARGET_TOPIC, rclcpp::SensorDataQoS(), std::bind(&AlignNode::targetCallback, this, std::placeholders::_1));
    ...
    pose_pub_ = this->create_publisher<geometry_msgs::msg::PoseWithCovarianceStamped>("/vgicp/pose", 10);
    ...
    this->declare_parameter("pcd_file", "/home/kazusahashimoto/ros2_ws/shinagawa_odaiba/output.pcd");
    ...
    this->declare_parameter("vgicp.resolution", 2.0);
    this->get_parameter("vgicp.resolution", resolution);
    this->declare_parameter("vgicp.max_iterations", 100);
    ...
    vgicp.setNeighborSearchMethod(fast_gicp::NeighborSearchMethod::DIRECT_RADIUS, resolution);
    vgicp.setDistanceMode(fast_gicp::NDTDistanceMode::P2D);
    vgicp.setDebugPrint(true);
    vgicp.clearTarget();
